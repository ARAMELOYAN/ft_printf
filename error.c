...
/*   Updated: 2022/11/18 12:59:42 by aeloyan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_rectangle_error(t_var *var, void (*f)(char *, t_var *))
{
	...
	if (!var->map[var->row])
		exit(1);
	var->col = ft_strlen(var->map[var->row]) - 1;
	var->quant.player = 0;
	...
	while (var->map[var->row])
	{
		...
		get_next_line(var);
		if (var->map[var->row]
			&& ft_strlen(var->map[var->row]) != (size_t)var->col + 1)
...
{
	if (str[0] != '1' || (str[1] && str[ft_strlen(str) - 2] != '1'))
		print_closed(var);
	while (str[++var->iter_i])
	{
		if (str[var->iter_i] != '1' && str[var->iter_i] != '\n')
		...
		if (str[var->iter_i] == 'P')
			...
			var->quant.exit++;
		else if (str[var->iter_i] == '0' || str[var->iter_i] == '\n'
